...
if USE_GEMINI:
    try:
        ...
        print("✅ AI Agent (v3) 初始化成功", flush=True)
...
# 區塊 2: 關鍵字映射 (核心邏輯)
# 說明:
# - 這是 v3 的核心，用於彌補 `items` 表沒有 `occasion` 或 `style` 欄位的問題。
# - `OCCASION_STYLE_MAPPING` 將使用者可能輸入的「場合」或「風格」關鍵字，
#   映射到一個或多個在 `items` 表中可以被查詢的 `clothing_type`。
...
#   - 產生 SQL 查詢，從 `items` 表中找出符合這些衣物類型的單品。
